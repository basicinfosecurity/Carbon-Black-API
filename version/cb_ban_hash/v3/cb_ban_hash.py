...
def main():
	...
	try:
		...
		if not opts.server_url or (not opts.server_url.startswith("http://") and not opts.server_url.startswith("https://")):
...
def export_to_csv():
	try:
		...
		with open("export_ban_list.csv", "wb") as export_csv:
			fieldnames = ['username', 'timestamp', 'user_id', 'enabled', 'text']
			#~ writer = csv.DictWriter(export_csv, fieldnames=fieldnames)
			writer = csv.writer(export_csv)
			#~ writer.writeheader()
			writer.writerow(fieldnames)
			for item in ban_list:
				#~ print(item.audit[0]['username'])
				writer.writerow([unicode(item.audit[0][f]).encode('utf-8') for f in fieldnames])
		export_csv.close()
...
class InvalidApiTokenError(CredentialError):
	def __init__(self, expression, message):
		self.expression = expression
		self.message = message
		self.exit_code = 200

class InvalidMD5Error(ApiError):
	def __init__(self, expression, message):
		self.expression = expression
		self.message = message
		self.exit_code = 300

class InvalidCommentError(ApiError):
	def __init__(self, expression, message):
		self.expression = expression
		self.message = message
		self.exit_code = 400
